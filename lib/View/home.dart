...
class _HomepageState extends State<Homepage> {
  ...
  final TextEditingController _textController =
      TextEditingController(); // TextController 만들기
  ...
  Future<void> getBookmark(BuildContext context) async {
    // 데이터 가져오기
      ...
      appBar: AppBar(
        title:
          ...
          Text(
            'Bible',
            style: TextStyle(
                color: Color.fromRGBO(253, 250, 245, 1.0),
        ...
        children: [
          ...
          Expanded(
            child: ListView(
              children: [
                SizedBox(height: 15,),
                bookmarkBible(deviceHeight, deviceWidth),
                SizedBox(height: 15,),
                momentBible(deviceHeight, deviceWidth),
                SizedBox(height: 15,)
              ],
            ),
          )
  ...
  String getRandomWordsOfBible() {
    ...
    int bibleChapter =
        Random().nextInt(bibleLength[bibleName]!); // 랜덤한 chapter 수 선택

    Map<String, dynamic>? selectedBible = bibleEnglishName[bibleName];

    List<dynamic> result =
        selectedBible!["${bibleChapter + 1}장"]; // 장에 맞는 절들 가져오기

    int bibleVerseLength =
        Random().nextInt(selectedBible!["${bibleChapter + 1}장"].length);
  ...
  double getAnimateContainerHeight() {
    // 높이 조절
    double result = 0.0;
  ...
  Widget header(double deviceHeight) {
    return Material(
      elevation: 2,
      ...
      child: Column(
        children: [searchBible(deviceHeight), searchResult(deviceHeight)],
      ),
    );
  }

  Widget searchBible(double deviceHeight) {
    return Container(
          ...
          if (value.contains(" ")) {
            ...
            _textController.selection = TextSelection.fromPosition(TextPosition(
                offset: _textController
                    .text.length)); // textfield 값 변경후, 단어 맨 뒤로 포커싱 해주기
          }
  ...
  Widget searchResult(double deviceHeight) {
    return Container(
              ...
              return GestureDetector(
                  ...
                  onTap: () {
                    ...
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) => SelectBibleChapter(
                              bibleTitle: bibleTitle,
                              chapterLength: getChapter(bibleTitle),
                            )));
                    _textController.clear();
                    setState(() {
                      searchList = [];
  ...
  Widget bookmarkBible(double deviceHeight, double deviceWidth) {
    return Container(
      ...
      child: Column(
        children: [
              ...
              children: [
                const Text(
                  ...
                  style: TextStyle(
                      fontSize: 25,
                      fontWeight: FontWeight.bold,
                      color: Color.fromRGBO(5, 35, 44, 1.0)),
                ),
                context.watch<BookMarkList>().results.isNotEmpty
                    ? IconButton(
                        onPressed: () {
                          showDialog(
                              context: context,
                                          ...
                                          if ((_pageController.page)!.toInt() ==
                                            ...
                                            if (currentPage == 0) {
                                              ...
                                              currentPage = (_pageController
                                                          .page)!
                                                      .toInt() -
                                                  1; // 마지막 페이지에서 북마크 제거시 currentPage - 1해줘야 맞음
                                            }
                                          } else {
                                            currentPage = (_pageController
                                                    .page)!
                                                .toInt(); // 첫번째나 중간페이지 제가한 경우에는 -1 하지 않아야 currentPage를 맞춤
                                          }
                                        });
                                      },
                                      child: const Text(
          ...
          SizedBox(
              ...
              child: context.watch<BookMarkList>().results.isNotEmpty
                  ? PageView.builder(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: Text(
                                  "${context.watch<BookMarkList>().results[index]["content"]}",
                                  style: const TextStyle(
                                      fontSize: 18,
                                      color: Color.fromRGBO(5, 35, 44, 1.0)),
                                  maxLines: 7,
                              ...
                              Text(
                                "${context.watch<BookMarkList>().results[index]['bible']} ${context.watch<BookMarkList>().results[index]['chapter'] + 1}:${context.watch<BookMarkList>().results[index]['verse']} KRV",
                                style: const TextStyle(
                                    fontSize: 13,
                                    fontWeight: FontWeight.bold,
                                    color: Color.fromRGBO(5, 35, 44, 1.0)),
                      ...
                      itemCount: context.watch<BookMarkList>().results.length,
                      onPageChanged: (page) {
                        setState(() {
                          currentPage = page;
                        });
          ...
          context.watch<BookMarkList>().results.isNotEmpty
              ? Text(
                  '${currentPage + 1} / ${context.watch<BookMarkList>().results.length}',
  ...
  Widget momentBible(double deviceHeight, double deviceWidth) {
    return Container(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: Text(
                    randomBibleWord,
                    style: const TextStyle(
                        fontSize: 18, color: Color.fromRGBO(5, 35, 44, 1.0)),
                    maxLines: 5,
                  ),
                ),
                const SizedBox(
                ...
                Text(
                  "$randomBible ${int.parse(randomChapter) + 1}:${int.parse(randomVerse) + 1} KRV",
                  style: const TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.bold,
                      color: Color.fromRGBO(5, 35, 44, 1.0)),
